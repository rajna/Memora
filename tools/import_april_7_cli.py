#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Import April 7, 2026 conversations to Memora"""
import json
import sys
...
def is_april_7(ts):
    try:
        ...
        return dt.year == 2026 and dt.month == 4 and dt.day == 7
    except:
        return False
...
print("1. Parsing April 7 sessions...")
...
for i, session in enumerate(conversations, 1):
    try:
        node = ms.add_memory_from_messages(
            messages=session,
            source="cli-april-7",
            base_tags=["cli-import", "2026-04-07"]
        )
        if node:
            imported += 1
