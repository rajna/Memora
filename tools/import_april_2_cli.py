#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Import April 2, 2026 conversations to Memora"""
import json
import sys
import time
...
def is_april_2(ts):
    try:
        ...
        return dt.year == 2026 and dt.month == 4 and dt.day == 2
    except:
        return False
...
print("1. Parsing April 2 sessions...")
...
for i, session in enumerate(conversations, 1):
    try:
        node = ms.add_memory_from_messages(
            messages=session,
            source="cli-april-2",
            base_tags=["cli-import", "2026-04-02"]
        )
        if node:
            imported += 1
            if i % 20 == 0:
                print("   {}/{}...".format(i, len(conversations)))
